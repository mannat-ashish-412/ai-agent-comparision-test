...
def save_state_to_file(state: AgentState, output_dir: str = "./outputs") -> Path:
    ...
    state_dict["start_time"] = state["start_time"].isoformat() if "start_time" in state else None
...
def generate_task_graph_visualization(tasks: list[Task], output_file: str = "task_graph.dot") -> str:
    """
    ...
    for task in tasks:
        ...
        label = f"{task.id}\\n{task.title}"
        dot_content.append(f'    "{task.id}" [label="{label}", fillcolor={color}, style=filled];')
...
def get_task_statistics(tasks: list[Task]) -> dict[str, Any]:
    ...
    return {
        ...
        "success_rate": round(status_counts.get("completed", 0) / len(tasks) * 100, 2) if tasks else 0
    }
...
def format_execution_summary(state: AgentState) -> str:
    ...
    execution_time = (datetime.utcnow() - state["start_time"]).total_seconds() if "start_time" in state else 0
