...
def evaluate(actual_output: Any, expected_output: Dict[str, Any]) -> Dict[str, float]:
        ...
        if len(conflicts) > 0:
            ...
            for conflict in conflicts:
                if isinstance(conflict, dict):
                    has_sources = any(k in conflict for k in ["source1", "source2", "sources"])
                    has_values = any(k in conflict for k in ["value1", "value2", "values"])
    ...
    if "audit_trail" in actual_output:
        ...
        if any(word in audit for word in ["because", "reason", "chose", "selected", "policy"]):
            trace_points += 30
